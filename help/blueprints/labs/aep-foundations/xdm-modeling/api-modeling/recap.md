...
O vídeo abaixo recapitula como você criou o esquema, as identidades e os descritores de relacionamento por meio de chamadas de API e demonstra como o patch de JSON é usado para modificar um esquema.

>[!VIDEO](https://video.tv.adobe.com/v/3459564/?quality=12&learn=on)

>[!SUCCESS]
>
>Parabéns! Entender como isso funciona ajuda você a entender o sistema como um todo.
...
## Esquema da conta do cliente corrigido em JSON
...
Para criar `Identity Descriptors` para os campos `_devbc.customerID` e `personalEmail.address` no esquema Conta de Cliente, você executou duas das mesmas chamadas `POST`.
...
>[!NOTE]
>
>O descritor `referenceIdentity` informa ao Perfil de cliente em tempo real qual campo no esquema `Customer Account` corresponde a qual namespace de identidade. Lembre-se de que, ao definir um esquema de pesquisa, você deve marcar um campo como uma identidade primária e atribuir a ele um namespace com um tipo de `non-person`.
